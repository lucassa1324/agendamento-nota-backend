		...
		publicGroup
			.get(
				...
				async ({
					...
					try {
						console.log(
							`>>> [BACK_PUBLIC_ACCESS] Listando agendamentos (público) para empresa: ${companyId}`,
						);
						...
						const isSlotRequest =
							startDateStr &&
							endDateStr &&
							startDateStr.split("T")[0] === endDateStr.split("T")[0];
						...
						if (isSlotRequest && startDate) {
							console.log(
								`>>> [SLOT_GENERATION] Gerando slots para ${startDateStr.split("T")[0]}`,
							);

							const settingsUseCase = new GetOperatingHoursUseCase(
								businessRepository,
							);
							...
							const dayConfig = settings.weekly.find(
								(w: any) =>
									...
									const isOccupied = appointments.some((app) => {
										...
										if (app.serviceDurationSnapshot) {
											if (app.serviceDurationSnapshot.includes(":")) {
												const [dH, dM] = app.serviceDurationSnapshot
													.split(":")
													.map(Number);
					...
					try {
						...
						if (isNaN(scheduledAt.getTime())) {
							...
							return {
								...
								message:
									"A data de agendamento fornecida é inválida. Use o formato ISO (ex: 2025-12-24T10:00:00Z)",
							};
						}
						...
						const result = await createAppointmentUseCase.execute(
							...
							user?.id,
						); // Passa user?.id (pode ser undefined se público)

						return result;
					} catch (error: any) {
						console.error(
							"[APPOINTMENT_CONTROLLER] Erro ao criar agendamento:",
							error,
						);

						const errorMessage =
							error.message || "Erro interno ao criar agendamento";
		...
		privateGroup
			...
			.get(
				...
				async ({
					...
					try {
						console.log(
							`>>> [BACK_ADMIN_ACCESS] Listando agendamentos (admin) para empresa: ${companyId}`,
						);
						...
						console.log(
							`>>> [FILTRO_DATA] Start: ${startDateStr}, End: ${endDateStr}`,
						);
						...
						const results = await listAppointmentsUseCase.execute(
							companyId,
							user!.id,
							startDate,
							endDate,
						...
						console.log(
							`>>> [ADMIN_RESULTS] Encontrados ${results.length} agendamentos`,
						);

						return results || [];
					} catch (error: any) {
						console.error(`>>> [ADMIN_ERROR]`, error.message);
						set.status = error.message.includes("Unauthorized access")
							? 403
							: 500;
			...
			.patch(
				...
				async ({
					...
					try {
						...
						return await updateAppointmentStatusUseCase.execute(
							id,
							body.status,
							user!.id,
						);

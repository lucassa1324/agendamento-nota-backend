...
export class CreateAppointmentUseCase {
	...
	async execute(data: CreateAppointmentInput, userId?: string) {
		...
		if (userId && business.ownerId !== userId) {
			throw new Error(
				"Unauthorized: Only business owners can create manual appointments",
			);
		}
		...
		for (const sId of serviceIds) {
			...
			if (service.companyId !== data.companyId) {
				throw new Error(
					`Service ${service.name} does not belong to this company`,
				);
			}
		...
		data.notes = data.notes
			? `${data.notes} | IDs: ${originalServiceId}`
			: `IDs: ${originalServiceId}`;
		...
		const settings = await this.businessRepository.getOperatingHours(
			data.companyId,
		);
		if (!settings) {
		...
		const checkTimeInPeriod = (
			...
			return (
				appTimeTotalMin >= startMin &&
				appTimeTotalMin + totalDurationMin <= endMin
			);
		};

		const isInMorning = checkTimeInPeriod(
			dayConfig.morningStart,
			dayConfig.morningEnd,
		);
		const isInAfternoon = checkTimeInPeriod(
		...
		const existingAppointments =
			await this.appointmentRepository.findAllByCompanyId(
				data.companyId,
		...
		const hasConflict = existingAppointments.some((app) => {
			...
			const existingStart =
				app.scheduledAt.getHours() * 60 + app.scheduledAt.getMinutes();
		...
		try {
			...
			if (owner && owner.notifyNewAppointments) {
				...
				console.log(
					`[WEBPUSH] Notificação de agendamento enviada para ${owner.email}`,
				);
			}
		} catch (notifyError: any) {
			console.error(
				"[WEBPUSH_TRIGGER_ERROR]",

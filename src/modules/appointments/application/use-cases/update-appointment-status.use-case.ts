		...
		const updatedAppointment = await db.transaction(async (tx) => {
             ...
             OR (${inventoryLogs.reason} LIKE ${`%Agendamento #${id} revertido%`} AND ${inventoryLogs.type} = 'ENTRY')`,
				...
				if (allLogs.length > 0) {
					...
					const balanceMap = new Map<
						string,
						{ product: typeof inventory.$inferSelect; balance: number }
					>();

					for (const { log, product } of allLogs) {
						const current = balanceMap.get(log.inventoryId) || {
							product,
							balance: 0,
						};
					...
					for (const [
						...
						if (balance > 0.0001) {
							...
							console.log(
								`[SKIP] Item: ${product.name} | Saldo já está zerado ou negativo (${balance}).`,
							);
						}
					}
				} else {
					...
					console.log(
						"⚠️ ATENÇÃO: Nenhum log encontrado. O sistema vai cair no Fallback.",
					);
					...
					const existingLog = await tx
						...
						.from(inventoryLogs)
						.where(
							sql`${inventoryLogs.reason} LIKE ${`%Agendamento #${id} revertido%`}`,
						)
						.limit(1);

					if (existingLog.length > 0) {
						console.log(
							"[FALLBACK SKIP] Já existe estorno para este agendamento legado.",
						);
					} else {
						...
						const resources = await tx
							...
							.innerJoin(
								inventory,
								eq(serviceResources.inventoryId, inventory.id),
							)
						...
						const uniqueSharedResources = new Map<
							string,
							{
								resource: typeof serviceResources.$inferSelect;
								product: typeof inventory.$inferSelect;
							}
						>();
						...
						for (const item of resources) {
							const isShared =
								...
								(item.product.isShared as any) === "true";
							if (isShared) {
								const existing = uniqueSharedResources.get(
									item.resource.inventoryId,
								);
								if (
									!existing ||
									Number(item.resource.quantity) >
										Number(existing.resource.quantity)
								) {
						...
						const itemsToRevert = [
							...Array.from(uniqueSharedResources.values()),
							...nonSharedResources,
						];
						...
						for (const { resource, product } of itemsToRevert) {
							...
							if (
								...
								resource.unit === product.secondaryUnit &&
								conversionFactor > 0
							) {
			...
			if (currentAppointment.status !== "COMPLETED" && status === "COMPLETED") {
				...
				for (const { resource, product } of itemsToConsume) {
					...
					console.log(
						`Testando alerta: Saldo atual ${newQty} ${product.unit} -> ${comparisonQty} ${product.secondaryUnit || product.unit} | Limite (Config) ${minQty} ${product.unit} -> ${comparisonMin} ${product.secondaryUnit || product.unit}`,
					);

					if (
						comparisonQty <= comparisonMin &&
						!notifiedLowStock.has(product.id)
					) {
						try {
							...
							console.error(
								"[INVENTORY_ALERT] Error sending notification:",
								err,
							);
						}
					}

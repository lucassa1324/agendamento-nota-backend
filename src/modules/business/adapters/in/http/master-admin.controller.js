...
export const masterAdminController = new Elysia({ prefix: "/admin/master" })
	...
	.get("/users", async () => {
		try {
			const results = await db
				...
				.leftJoin(
					schema.companies,
					eq(schema.user.id, schema.companies.ownerId),
				);
			return results;
		} catch (error) {
			...
			try {
				...
				if (actionType) {
					...
					if (actionType === "manual_custom_days") {
						...
						console.log(
							`[MASTER_ADMIN] Reset Automático: Trial zerado e acesso bloqueado (sem pagamento).`,
						);
					}
				}
				const [updated] = await db
	...
	.get("/users/:id/details", async ({ params, set }) => {
		try {
			...
			const [appointmentStats] = await db
				...
				.from(schema.appointments)
				.where(
					company ? eq(schema.appointments.companyId, company.id) : undefined,
				);
	...
	.delete("/users/:id", async ({ params, set }) => {
		try {
			...
			console.log(
				`[MASTER_ADMIN_DELETE]: Usuário ${deleted.email} e todos os seus dados foram removidos.`,
			);
			return {
				success: true,
			...
			try {
				...
				if (active === false) {
					try {
						...
						console.log(
							`[MASTER_ADMIN_KICK]: Sessões invalidadas para o estúdio ${updated.name} (Owner: ${updated.ownerId})`,
						);
					} catch (kickError) {

...
export const publicBusinessController = new Elysia({ prefix: "/api/business" })
	...
	.get(
		...
		async ({
			...
			console.log(
				`[PUBLIC_BUSINESS_FETCH] Buscando dados para o slug: ${slug}`,
			);
			...
			if (!business) {
				console.log(
					`[PUBLIC_BUSINESS_FETCH] Empresa não encontrada para o slug: ${slug}`,
				);
				set.status = 404;
			...
			console.log(
				`[PUBLIC_BUSINESS_FETCH] Dados retornados do banco para o slug: ${slug}`,
			);
			...
			if (!publicEmail && business.ownerId) {
				...
				try {
					...
					console.error(
						`[PUBLIC_BUSINESS_FETCH] Erro ao buscar owner para fallback de email:`,
						err,
					);
				}
			}
			...
			const primaryColor =
				customization?.layoutGlobal?.siteColors?.primary ||
				customization?.layoutGlobal?.base_colors?.primary ||
				"Padrão";

			console.log(
				`[PUBLIC_BUSINESS_FETCH] Cor Primária no Banco:`,
				primaryColor,
			);
			console.log(
				`[PUBLIC_BUSINESS_FETCH] Contato resolvido - Email: ${publicEmail}, Phone: ${publicPhone}`,
			);

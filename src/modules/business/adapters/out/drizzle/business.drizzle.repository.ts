...
export class DrizzleBusinessRepository implements IBusinessRepository {
	async findAllByUserId(userId: string): Promise<BusinessSummary[]> {
		const results = await db
			...
			.leftJoin(
				companySiteCustomizations,
				eq(companies.id, companySiteCustomizations.companyId),
			)
	...
	async create(data: CreateBusinessInput): Promise<Business> {
		return await db.transaction(async (tx) => {
			...
			const [newCustomization] = await tx
	...
	async updateConfig(
		id: string,
		userId: string,
		config: Partial<BusinessSiteCustomization>,
	...
	async getOperatingHours(
		...
		const [company] = await query.where(and(...conditions)).limit(1);
		...
		const appointmentFlow =
			(company.siteCustomization?.appointmentFlow as any) || {};
		// Suportar tanto a chave antiga quanto a nova (plural) e as variações de snake/camel case
		const step3 =
			appointmentFlow.step3Times ||
			appointmentFlow.step3Time ||
			appointmentFlow.step_3_time ||
			{};
		let slotInterval =
			step3.timeSlotSize ||
			step3.slot_interval ||
	...
	async deleteAgendaBlock(
		...
		return await db.transaction(async (tx) => {
			...
			const result = await tx
				...
				.where(
					and(
						eq(agendaBlocks.id, blockId),
						eq(agendaBlocks.companyId, companyId),
					),
				)
				.returning();

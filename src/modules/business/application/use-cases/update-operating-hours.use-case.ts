...
export class UpdateOperatingHoursUseCase {
	...
	async execute(
		...
		const ok = await this.businessRepository.setOperatingHours(
			companyId,
			userId,
			normalized,
		);
		...
		let slotInterval =
			...
			(data as any).slotInterval ??
			(data as any).timeInterval;
		...
		console.log(
			`>>> [BACK_SAVE_SETTINGS] Salvando intervalo: ${slotInterval} para a empresa: ${companyId}`,
		);
		...
		const config: Partial<BusinessSiteCustomization> = {
			appointmentFlow: {
				...currentFlow,
				step3Times: {
					...(currentFlow.step3Times ||
						currentFlow.step3Time ||
						currentFlow.step_3_time ||
						{}),

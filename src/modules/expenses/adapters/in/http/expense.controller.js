...
export const expenseController = new Elysia({ prefix: "/expenses" })
	...
	.onBeforeHandle(({ user, set, request }) => {
		console.log(
			`>>> [AUTH_CHECK] Tentativa de acesso a ${request.method} ${request.url}`,
		);
		if (!user) {
	...
	.post(
		"/",
		async ({ body, expenseRepository, user, set }) => {
	...
	.get(
		"/",
		async ({ query, expenseRepository, user, set }) => {
			const businessId = user.businessId;
	...
	.patch(
		"/:id",
		async ({ params: { id }, body, expenseRepository, user, set }) => {
			const businessId = user.businessId;
			if (!businessId) {
				set.status = 403;
				return { error: "Não autorizado" };

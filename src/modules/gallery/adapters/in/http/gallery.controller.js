...
export const galleryController = new Elysia({ prefix: "/gallery" })
	...
	.group("/public", (app) =>
		app.get(
			"/:businessId",
			async ({ params: { businessId }, query, galleryRepository, set }) => {
				try {
					const filters = {
						...
						showInHome:
							query.showInHome === "true"
								? true
								: query.showInHome === "false"
									? false
									: undefined,
					};
					const images = await galleryRepository.findByBusinessId(
						businessId,
						filters,
					);
					return images;
		...
		app
			...
			.get("/categories", async ({ serviceRepository, user, set }) => {
				try {
					...
					const services =
						await serviceRepository.findAllByCompanyId(businessId);
			...
			.post(
				"/",
				async ({ body, galleryRepository, user, set }) => {
					try {
						const businessId = user.businessId;
						if (!businessId) {
							set.status = 401;
			...
			.patch(
				"/:id",
				async ({ params: { id }, body, galleryRepository, user, set }) => {
					try {
			...
			.delete(
				"/:id",
				async ({ params: { id }, galleryRepository, user, set }) => {
					try {
						const existing = await galleryRepository.findById(id);
						if (!existing) {
							set.status = 404;

		...
		try {
			...
			if (user && user.id) {
				try {
					...
					if (userCompany) {
						console.log(
							`>>> [AUTH_DEBUG] Business Found -> ID: ${userCompany.id} | Slug: ${userCompany.slug} | Active: ${userCompany.active}`,
						);
					} else {
					...
					if (
						...
						console.warn(
							`[AUTH_BLOCK]: Conta de usuário desativada: ${user.email}`,
						);
						set.status = 403;
					...
					if (userCompany) {
						...
						if (
							...
							console.warn(
								`[AUTH_BLOCK]: Acesso negado para estúdio suspenso: ${userCompany.slug} (User: ${user.email})`,
							);
							set.status = 403;
						...
						if (!isMasterRoute && !isAuthRoute && user.role !== "SUPER_ADMIN") {
							...
							const trialEnds = userCompany.trialEndsAt
							...
							if (
								...
								console.warn(
									`[AUTH_BLOCK]: Acesso negado por status inválido (${status}): ${userCompany.slug}`,
								);
								set.status = 402;
							...
							if (isManualActive && trialEnds && trialEnds <= now) {
								console.warn(
									`[AUTH_UPDATE]: Acesso Manual expirado para ${userCompany.slug}. Revertendo para 'past_due' (Automático).`,
								);
								db.update(schema.companies)
									...
									.then(() =>
										console.log(
											`[AUTH_UPDATE_SUCCESS]: Reset de Manual para Automático/PastDue - ${userCompany.slug}`,
										),
									)
									.catch((err) =>
										console.error(
											`[AUTH_UPDATE_ERROR]: Falha ao resetar status - ${userCompany.slug}`,
											err,
										),
									);
							...
							if (status === "trial" && trialEnds && trialEnds <= now) {
								console.warn(
									`[AUTH_UPDATE]: Trial expirado para ${userCompany.slug}. Atualizando status para 'past_due'.`,
								);
								...
								db.update(schema.companies)
									...
									.then(() =>
										console.log(
											`[AUTH_UPDATE_SUCCESS]: Status atualizado para 'past_due' - ${userCompany.slug}`,
										),
									)
									.catch((err) =>
										console.error(
											`[AUTH_UPDATE_ERROR]: Falha ao atualizar status - ${userCompany.slug}`,
											err,
										),
									);
							...
							if (!isManualActive && !isActive && !isTrialValid) {
								console.warn(
									`[AUTH_BLOCK]: Acesso negado por falta de pagamento/trial expirado: ${userCompany.slug} (Status: ${status})`,
								);
					...
					if (
						dbError.message === "BUSINESS_SUSPENDED" ||
						dbError.message === "BILLING_REQUIRED"
					) {
						throw dbError;
					}
			...
			if (
				error.message === "BUSINESS_SUSPENDED" ||
				error.message === "ACCOUNT_SUSPENDED" ||
				error.message === "BILLING_REQUIRED"
			) {
				throw error;
			}

	...
	hooks: {
		before: async (context: any) => {
			...
			if (path.includes("/sign-out")) {
				try {
					...
					console.error(
						"[AUTH_BEFORE_HOOK] Erro ao verificar sessão no sign-out:",
						e,
					);
				}
			}
		...
		after: async (context: any) => {
			...
			try {
				...
				if (!response) {
					// DO NOT call auth.api.getSession here again! It causes infinite recursion.
					...
					try {
						...
						if (userCompany) {
							...
							if (userCompany.active === false && user.role !== "SUPER_ADMIN") {
								console.warn(
									`[AUTH_BLOCK]: Estúdio suspenso - ${userCompany.slug}`,
								);
							...
							console.log(
								`[AUTH_AFTER_HOOK] Enriquecido com sucesso: ${user.email} -> ${userCompany.slug}`,
							);
						} else {
							console.log(
								`[AUTH_AFTER_HOOK] Nenhuma empresa encontrada para: ${user.email}`,
							);
						}
					} catch (dbError) {
						...
						const results = await db
							.select({
								...
								siteCustomization: {
									...
									appointmentFlow:
										schema.companySiteCustomizations.appointmentFlow,
								},
							})
							.from(schema.companies)
							.leftJoin(
								...
								eq(
									schema.companies.id,
									schema.companySiteCustomizations.companyId,
								),
							)

...
export const asaasWebhookController = new Elysia({
	prefix: "/webhook/asaas",
	...
	try {
		...
		console.log(
			`[ASAAS_WEBHOOK] Processando pagamento para Customer: ${customerId}`,
		);
		if (
			...
			if (externalReference) {
				const paymentDate = event.payment.paymentDate
				...
				console.log(
					`[ASAAS_WEBHOOK] Pagamento confirmado! Empresa ${externalReference} ativada até ${nextDue.toISOString()}.`,
				);
			} else {
				console.warn(
					"[ASAAS_WEBHOOK] ExternalReference ausente, não foi possível identificar a empresa.",
				);
			}
		} else if (
			...
			if (externalReference) {
				await db
					...
					.where(eq(companies.id, externalReference));
				console.log(
					`[ASAAS_WEBHOOK] Pagamento pendente/estornado. Empresa ${externalReference} marcada como past_due.`,
				);
			}
		}

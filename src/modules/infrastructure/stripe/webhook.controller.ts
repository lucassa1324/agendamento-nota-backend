...
export const stripeWebhookController = new Elysia({ prefix: "/stripe" }).post(
	"/webhook",
	async ({ request, set }) => {
			...
			switch (event.type) {
				...
				case "customer.subscription.updated": {
					...
					const company = await db
						.select()
						.from(companies)
						.where(eq(companies.stripeCustomerId, customerId as string))
						.limit(1);
					...
					if (company.length > 0) {
						...
						if (currentCompany.accessType === "manual") {
							console.log(
								`[STRIPE_WEBHOOK] Skipping update for manual company ${currentCompany.id}`,
							);
						} else {
							await db
								.update(companies)
							...
							console.log(
								`[STRIPE_WEBHOOK] Company ${currentCompany.id} updated to ${newStatus}`,
							);
						}
					}
					break;
					...
					if (company.length > 0) {
						...
						if (currentCompany.accessType !== "manual") {
							await db
								...
								.where(eq(companies.id, currentCompany.id));
							console.log(
								`[STRIPE_WEBHOOK] Company ${currentCompany.id} canceled.`,
							);
						}
					}
					break;

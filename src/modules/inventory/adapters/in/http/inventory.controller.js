...
export const inventoryController = new Elysia({ prefix: "/inventory" })
	...
	.post(
		"/",
		async ({ body, inventoryRepository, businessRepository, user, set }) => {
			try {
				console.log("Dados recebidos no POST /api/inventory:", body);
				console.log(
					`[INVENTORY_CONTROLLER] Criando produto para empresa: ${body.companyId}`,
				);
	...
	.get(
		...
		async ({
			...
			try {
				console.log(
					`[INVENTORY_CONTROLLER] Listando produtos para empresa: ${companyId}`,
				);
	...
	.patch(
		...
		async ({
			...
			try {
				...
				// @ts-ignore
				console.log(
					`[INVENTORY_CONTROLLER] isShared recebido:`,
					body.isShared,
					typeof body.isShared,
				);
	...
	.post(
		...
		async ({
			...
			try {
				console.log(
					`[INVENTORY_CONTROLLER] Subtraindo estoque para o produto ${id}:`,
					body,
				);
	...
	.post(
		...
		async ({
			...
			try {
				console.log(
					`[INVENTORY_CONTROLLER] Nova transação de estoque recebida:`,
					body,
				);
				const useCase = new CreateInventoryTransactionUseCase(
					inventoryRepository,

...
export class DrizzleInventoryRepository implements InventoryRepository {
	...
	async findByCompanyId(companyId: string): Promise<Product[]> {
		try {
			...
			console.error(
				"[DRIZZLE_INVENTORY_REPOSITORY_FINDBYCOMPANYID_ERROR]:",
				error,
			);
			throw error;
		}
	...
	async update(
		id: string,
		data: Partial<
			Omit<Product, "id" | "companyId" | "createdAt" | "updatedAt">
		>,
	): Promise<Product> {
		try {
			console.log(`[DRIZZLE_INVENTORY_REPO] Iniciando update para ID: ${id}`);
			console.log(
				`[DRIZZLE_INVENTORY_REPO] Valores para update:`,
			...
			if (data.isShared !== undefined) {
				console.log(
					`[DRIZZLE_INVENTORY_REPO] Atualizando isShared para: ${data.isShared}`,
				);
				updateData.isShared = data.isShared;
			}

			console.log(
				`[DRIZZLE_INVENTORY_REPO] Objeto final enviado ao Drizzle .set():`,
			...
			if (!result) {
				console.error(
					`[DRIZZLE_INVENTORY_REPO] Produto com ID ${id} não encontrado para update.`,
				);
				throw new Error("Product not found");
			}

			console.log(
				`[DRIZZLE_INVENTORY_REPO] Update realizado com sucesso. Resultado:`,
	...
	async createLog(
		log: Omit<InventoryLog, "id" | "createdAt">,
	...
	async getLogsByCompany(companyId: string): Promise<InventoryLog[]> {
		try {
			...
			console.error(
				"[DRIZZLE_INVENTORY_REPOSITORY_GETLOGS_BY_COMPANY_ERROR]:",
				error,
			);
			throw error;
		}

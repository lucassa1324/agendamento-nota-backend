...
export const serviceController = new Elysia({ prefix: "/services" })
	...
	.get(
		"/company/:companyId",
		async ({ params: { companyId }, serviceRepository, set }) => {
			try {
				console.log(
					`>>> [BACK_PUBLIC_ACCESS] Serviços liberados para a empresa: ${companyId}`,
				);
	...
	.group("", (app) =>
		app
			...
			.post(
				"/",
				async ({ body, serviceRepository, user, set }) => {
					try {
						console.log(
							`\n>>> [BACK_RECEIVED] POST /api/services:`,
						...
						console.log(
							`[SERVICE_CONTROLLER] Serviço processado com sucesso: ${result.id}`,
						);
						return result;
					} catch (error) {
			...
			.put(
				"/:id",
				async ({ params: { id }, body, serviceRepository, user, set }) => {
					try {
						console.log(
							`\n>>> [BACK_RECEIVED] PUT /api/services/${id}:`,
							JSON.stringify(body, null, 2),
						);
						const businessId = user.businessId;
						...
						console.log(
							`>>> [BACK_RECEIVED] Body normalizado para o repositório:`,
			...
			.delete(
				"/:id",
				async ({ params: { id }, serviceRepository, user, set }) => {
					try {
			...
			.get(
				"/check-exists/:id",
				async ({ params: { id }, serviceRepository, set }) => {
					try {
						console.log(
							`[SERVICE_CONTROLLER] Verificando existência do serviço com ID: ${id}`,
						);
						const exists = await serviceRepository.checkServiceExists(id);
						console.log(
							`[SERVICE_CONTROLLER] Serviço com ID ${id} existe: ${exists}`,
						);
						return { id, exists };
					} catch (error) {

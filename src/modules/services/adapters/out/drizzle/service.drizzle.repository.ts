...
export class DrizzleServiceRepository implements IServiceRepository {
	...
	async create(data: CreateServiceInput): Promise<Service> {
		...
		const serviceId =
			data.id && isUUID(data.id) ? data.id : crypto.randomUUID();

		console.log(
			`[DrizzleServiceRepository] Executando Upsert para ID: ${serviceId}`,
		);
		...
		if (rawRules) {
			if (rawRules.conflicts && Array.isArray(rawRules.conflicts)) {
				...
				finalAdvancedRules = rawRules.conflicts
					? { conflicts: rawRules.conflicts }
					: { conflicts: [], ...rawRules };
			}
		}
	...
	async update(
		...
		console.log(
			`[DrizzleServiceRepository] DADOS RECEBIDOS:`,
		...
		if (data.advancedRules !== undefined) {
			...
			console.log(
				`[DrizzleServiceRepository] AdvancedRules mapeado para:`,
				JSON.stringify(finalRules),
			);
		}

		console.log(
			`[DrizzleServiceRepository] PAYLOAD FINAL ENVIADO AO DRIZZLE:`,
		...
		try {
			...
			if (!updated) {
				console.warn(
					`[DrizzleServiceRepository] !!! NENHUM REGISTRO ENCONTRADO PARA O ID: ${id}`,
				);
				return null;
			}
			...
			if (data.resources !== undefined) {
				console.log(
					`[DrizzleServiceRepository] Atualizando recursos para o serviço: ${id}`,
				);
			...
			console.log(
				`[DrizzleServiceRepository] <<< UPDATE CONCLUÍDO COM SUCESSO!`,
			);
			...
			console.error(
				`[DrizzleServiceRepository] !!! ERRO FATAL NO UPDATE:`,
				error,
			);
			throw error;
		}

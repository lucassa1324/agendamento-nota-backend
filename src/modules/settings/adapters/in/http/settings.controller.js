...
export const settingsController = new Elysia({ prefix: "/settings" })
	...
	.get(
		...
		async ({ params: { businessId }, settingsRepository, set }) => {
			try {
				console.log(
					`>>> [BACK_PUBLIC_ACCESS] Customização liberada para a empresa: ${businessId}`,
				);
				...
				const getSiteCustomizationUseCase = new GetSiteCustomizationUseCase(
					settingsRepository,
				);
				const customization =
					await getSiteCustomizationUseCase.execute(businessId);
				console.log(
					`>>> [BACK_PUBLIC_ACCESS] Payload de customização enviado. AppointmentFlow presente: ${!!customization.appointmentFlow}`,
				);
	...
	.get(
		...
		async ({
			...
			try {
				...
				let publicEmail = profile?.email || business.contact;
				...
				const standardizedProfile = {
					...
					// Contato e Endereço
					phone: profile?.phone || business.contact || null,
					email: publicEmail || null,
					address: profile?.address || business.address || null,
					createdAt: profile?.createdAt || null,
					updatedAt: profile?.updatedAt || null,
				};
				console.log(
					`>>> [SETTINGS_PROFILE_RESPONSE] Enviando perfil padronizado para businessId: ${businessId}`,
				);
	...
	.group("", (app) =>
		app
			...
			.post(
				"/logo",
				async ({ body, user, settingsRepository, businessRepository, set }) => {
					try {
						...
						const saveSettingsUseCase = new SaveSettingsUseCase(
							settingsRepository,
						);
						await saveSettingsUseCase.execute(businessId, {
							logoUrl: base64Logo,
						});
						console.log(
							`[SETTINGS_CONTROLLER] Logo salva no banco de dados como Base64 para businessId: ${businessId}`,
						);
						return {
							success: true,
							logoUrl: base64Logo,
				...
				async ({
					...
					try {
						...
						if (!business || business.ownerId !== user.id) {
							...
							return {
								error:
									"Você não tem permissão para alterar as configurações desta empresa.",
							};
						}
						const saveSettingsUseCase = new SaveSettingsUseCase(
							settingsRepository,
						);
						const updatedProfile = await saveSettingsUseCase.execute(
							businessId,
							body,
						);
			...
			.patch(
				...
				async ({
					...
					try {
						...
						console.log(
							"COR RECEBIDA DO FRONT:",
							body.layoutGlobal?.siteColors?.primary ||
								body.layout_global?.site_colors?.primary,
						);
						// Validar se o usuário é dono da empresa
						const business = await businessRepository.findById(businessId);
						if (!business || business.ownerId !== user.id) {
							...
							return {
								error:
									"Você não tem permissão para alterar a personalização desta empresa.",
							};
						}
						const getSiteCustomizationUseCase = new GetSiteCustomizationUseCase(
							settingsRepository,
						);
						const updateSiteCustomizationUseCase =
							new UpdateSiteCustomizationUseCase(
								settingsRepository,
								getSiteCustomizationUseCase,
							);
						return await updateSiteCustomizationUseCase.execute(
							businessId,
							body,
						);
					} catch (error) {
						...
						if (error.all) {
							console.log(
								...
								JSON.stringify(error.all, null, 2),
							);
						}
						set.status = 500;
						return { error: error.message };

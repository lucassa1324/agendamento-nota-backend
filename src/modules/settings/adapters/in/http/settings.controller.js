...
export const settingsController = new Elysia({ prefix: "/settings" })
    ...
    try {
        ...
        console.log('COR RECEBIDA DO FRONT:', body.layoutGlobal?.siteColors?.primary || body.layout_global?.site_colors?.primary);
        // Validar se o usuário é dono da empresa
        const business = await businessRepository.findById(businessId);
        if (!business || business.ownerId !== user.id) {
            set.status = 403;
        ...
        const getSiteCustomizationUseCase = new GetSiteCustomizationUseCase(settingsRepository);
        const updateSiteCustomizationUseCase = new UpdateSiteCustomizationUseCase(settingsRepository, getSiteCustomizationUseCase);

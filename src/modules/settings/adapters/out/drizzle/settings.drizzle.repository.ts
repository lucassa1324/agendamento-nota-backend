...
export class DrizzleSettingsRepository implements SettingsRepository {
	async findByBusinessId(businessId: string): Promise<BusinessProfile | null> {
		try {
			...
			console.error(
				"[DRIZZLE_SETTINGS_REPOSITORY_FINDBYBUSINESSID_ERROR]:",
				error,
			);
			throw error;
		}
	...
	async findCustomizationByBusinessId(
		businessId: string,
	): Promise<SiteCustomization | null> {
		try {
			const [result] = await db
				.select()
			...
			console.error(
				"[DRIZZLE_SETTINGS_REPOSITORY_FINDCUSTOMIZATION_ERROR]:",
				error,
			);
			throw error;
		}
	...
	async saveCustomization(
		...
		try {
			...
			if (existing) {
				console.log(
					`[DRIZZLE_REPOSITORY] Atualizando customização existente para companyId: ${businessId}`,
				);

				// Log de Persistência Temporário
				const newColor = (data as any).appointmentFlow?.step1Services
					?.cardConfig?.backgroundColor;
				if (newColor) {
					console.log(
						`>>> [DB_SAVE] Salvando cor ${newColor} para o campo appointmentFlow.step1Services.cardConfig.backgroundColor`,
					);
				}
				...
				if (!updated) {
					throw new Error(
						"Falha ao atualizar customização: nenhum registro retornado.",
					);
				}

				console.log(
					`>>> [DB_CONFIRM] Banco retornou objeto atualizado para ID: ${businessId}`,
				);
				return {
				...
				console.log(
					`[DRIZZLE_REPOSITORY] Criando nova customização para companyId: ${businessId}`,
				);
				const [created] = await db
				...
				if (!created) {
					throw new Error(
						"Falha ao criar customização: nenhum registro retornado.",
					);
				}
			...
			console.error(
				"[DRIZZLE_SETTINGS_REPOSITORY_SAVECUSTOMIZATION_ERROR]:",
				error,
			);
			throw error;
		}

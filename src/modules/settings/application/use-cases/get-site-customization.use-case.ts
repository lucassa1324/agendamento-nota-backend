...
export class GetSiteCustomizationUseCase {
	...
	async execute(businessId: string): Promise<SiteCustomization> {
		const customization =
			await this.settingsRepository.findCustomizationByBusinessId(businessId);
		...
		if (!customization) {
			console.log(
				`[GET_SITE_CUSTOMIZATION] Nenhum registro encontrado para businessId: ${businessId}. Criando padrão.`,
			);
			// Create the default record in the database
			return await this.settingsRepository.saveCustomization(
				businessId,
		...
		console.log(
			`[GET_SITE_CUSTOMIZATION] Registro encontrado no banco para businessId: ${businessId}. Realizando merge para evitar nulos.`,
		);
		 ...
		 */
		if (
			merged.appointmentFlow?.step1Services?.cardConfig?.backgroundColor ===
			"TRANSPARENT_DEFAULT"
		) {
			merged.appointmentFlow.step1Services.cardConfig.backgroundColor =
				"#F3E2E2"; // Fallback real
		}

		console.log(
			`>>> [GET_SITE_CUSTOMIZATION] Contrato Validado: step1Services.cardConfig.backgroundColor = ${merged.appointmentFlow?.step1Services?.cardConfig?.backgroundColor}`,
		);
		...
		if (merged.appointmentFlow?.step3Times) {
			...
			merged.appointmentFlow.step3Times.timeSlotSize =
				typeof size === "string" ? parseInt(size, 10) : Number(size || 30);

			console.log(
				">>> [PUBLIC_API_SEND] Enviando intervalo para o site:",
				merged.appointmentFlow.step3Times.timeSlotSize,
			);
		}
		 ...
		 */
		console.log(
			`>>> [BOOKING_DEBUG] Intervalo de agendamento (timeSlotSize) para businessId ${businessId}: ${merged.appointmentFlow?.step3Times?.timeSlotSize} min`,
		);

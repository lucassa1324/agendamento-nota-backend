export class UpdateSiteCustomizationUseCase {
	constructor(settingsRepository, getSiteCustomizationUseCase) {
		this.settingsRepository = settingsRepository;
		this.getSiteCustomizationUseCase = getSiteCustomizationUseCase;
	}
	async execute(businessId, partialData) {
		console.log(
			">>> [BACK_SAVE] Recebendo novos dados para ID:",
			businessId,
			partialData,
		);
		...
		console.log(
			`[UPDATE_SITE_CUSTOMIZATION] Objeto final após deep merge para businessId: ${businessId}`,
		);
		console.dir(merged, { depth: null, colors: true });
		const result = await this.settingsRepository.saveCustomization(
			businessId,
			merged,
		);
		console.log(
			`>>> [BACK_SAVE_SUCCESS] Dados persistidos com sucesso para ID: ${businessId}`,
		);
		return result;
	}
	normalizeKeys(obj) {
		...
		for (const key in obj) {
			...
			if (targetKey === "timeSlotSize" && typeof value === "string") {
				...
				if (!isNaN(parsed)) {
					console.log(
						`>>> [NORMALIZE] Convertendo timeSlotSize de string para número: ${value} -> ${parsed}`,
					);
					value = parsed;
				}
			}
			...
			if (key === "cardBgColor" || key === "card_bg_color") {
				console.log(
					`>>> [NORMALIZE] Remapeando ${key} para cardConfig.backgroundColor`,
				);

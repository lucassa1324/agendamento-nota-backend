...
export class CreateUserUseCase {
	...
	async execute(data: SigninDTO) {
			...
			const result = await db
				.transaction(async (tx) => {
					...
					await tx
						.update(account)
						.set({ scope: "ADMIN" })
						.where(eq(account.userId, alreadyExists.id));
		...
		console.log(
			`[USER_REGISTER_USE_CASE] Iniciando signUpEmail para: ${data.email}`,
		);
		...
		console.log(
			`[USER_REGISTER_USE_CASE] signUpEmail concluído com sucesso para: ${response.user.email}`,
		);
		...
		console.log(
			`[USER_REGISTER_USE_CASE] Role '${finalRole}' aplicada ao usuário ${response.user.id}`,
		);
		...
		const result = await db
			.transaction(async (tx) => {
				await tx
					.update(account)
					.set({ scope: "ADMIN" })
					.where(eq(account.userId, response.user.id));
